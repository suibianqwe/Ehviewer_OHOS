...
本教程适用于 EhViewer HarmonyOS `0.6.9`。不同设备的系统版本、窗口尺寸和权限状态可能让按钮位置略有差异，但功能入口和操作逻辑相同。竖屏截图来自手机，横屏截图来自平板；教程中的阅读器均从 `订阅 → 画廊详情 → 阅读` 进入。
...
登录成功后，应用会尝试自动打开当前 E-Hentai/ExHentai 站点的账号配置并回收 `uconfig`。每日签到也会在后台打开当前站点的 `news.php`，并带上已保存的 Cookie；若签到成功，前台仍会显示签到结果弹窗。如果画廊封面、阅读器图片或配额仍无法加载，请优先手动执行一次：
...
- 内嵌阅读器会覆盖在当前页面上方，底层列表不会卸载；独立阅读器仍使用单独窗口并保持全屏，不参与分栏。
- 宽屏画廊列表保持固定的左右边距，不会随窗口继续变宽而产生额外留白。
...
- 点击已选标签上的删除按钮：移除该条件。
- 点击上传者：按上传者进入新的下一级搜索。
- 返回：逐层恢复上一级搜索条件、列表位置和详情状态。搜索历史异步加载完成后，建议列表会自动刷新。
...
- 相似画廊/搜索封面：打开对应搜索结果。
- 下载：加入下载队列。
- 阅读：在当前页面上方打开内嵌阅读器；需要独立窗口时则进入全屏阅读器。
...
关闭详情或阅读器后，列表、滚动位置、分栏焦点和右下角工具栏会恢复到离开前的状态。

## 7. 阅读器

内嵌阅读器覆盖在当前页面上方，底层列表保持挂载；独立阅读器在单独窗口中全屏显示。点击画面中央显示或隐藏控制层，左右区域用于翻页；双击可快速缩放，双指可缩放，长按图片打开图片操作菜单。退出后会恢复列表位置和分栏状态。
...
按钮、浮动工具栏和弹窗使用高斯模糊材质。新版 HarmonyOS 对沉浸光感的支持范围受限，因此 API 26 及旧版设备使用可见的兼容材质，不会因为系统能力差异显示为全透明或灰色。
