...
本教程适用于 EhViewer HarmonyOS `0.6.1`。不同设备的系统版本、窗口尺寸和权限状态可能让按钮位置略有差异，但功能入口和操作逻辑相同。
...
<p align="center">
  <img src="images/drawer.jpg" width="760" alt="侧边菜单与页面入口" />
</p>
...
<p align="center">
  <img src="images/reader-settings.jpg" width="420" alt="阅读器选项浮窗" />
</p>
...
- 超分与 HDR 可同时启用；处理顺序固定为先超分、再转 HDR，结果会在预渲染范围内提前生成。
- 长图超过接口尺寸限制时会分段超分，再组合用于显示。
- 自动裁切白边/黑边：在预渲染阶段识别图片外围近似纯色区域，减少无效边缘占用，不修改原始文件。

图像增强依赖设备能力，无法处理的格式或设备不支持时会保留原图。长按保存图片时，优先保存当前实际显示的超分/HDR 处理结果。
...
传输时保持两台设备页面开启，完成前不要主动断开 Wi-Fi 直连。阅读进度同步不传输漫画 OCR 翻译结果，因此即使历史记录较多也能保持较小的数据量。
