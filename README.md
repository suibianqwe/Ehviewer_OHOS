...
- 当前版本：`0.6.9`
- 安装包：[`EhViewer_OHOS_0.6.9.hap`](https://github.com/suibianqwe/Ehviewer_OHOS/releases/download/v0.6.9/EhViewer_OHOS_0.6.9.hap)
- 目标 API：`26.0.0`
- 兼容 API：`6.0.0(20)`
...
- 每日签到改为后台打开 `news.php`，自动同步已保存的 Cookie；签到结果仍在前台保留弹窗显示。
- 内嵌阅读器覆盖在原页面上方，底层列表保持挂载；退出阅读器或详情后会恢复列表、滚动位置、分栏焦点和工具栏状态。
- 修复搜索页异步加载搜索历史后建议列表不刷新的问题。
- 受 HarmonyOS 新版 API 对沉浸光感的限制，按钮、浮动工具栏和弹窗统一使用可见的高斯模糊材质，并为 API 26 及旧版保留兼容回退。
- 统一宽屏画廊列表左右边距，避免窗口变宽后边缘留白异常。
